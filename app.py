...
Single-file Streamlit app for official-style deployment.
...
def _load_model():
    ...
    scaler, model = _generate_training_data()
...
with tab1:
    ...
    if st.button(tr("calculate"), use_container_width=True):
        ...
        st.markdown(
            f"""
            <div class="result-panel" style="border-left: 6px solid {color};">
                <div class="result-label">{city} — {tr('risk')}</div>
                <div class="result-status" style="color:{color};">{status}</div>
                <div class="result-number" style="color:{color};">{risk_pct}<span style="font-size:1.8rem;color:#cbd5e1;">%</span></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        saturation_value = round((soil_percent / 100.0) * river, 1)
        with m1:
            st.metric(tr("metric_snowmelt"), f"{snowmelt_value} mm/day")
        with m2:
            st.metric(tr("metric_precip7"), f"{precip7_value} mm")
        with m3:
        ...
        for r in rain_grid:
            ...
            with torch.no_grad():
                p = torch.sigmoid(model(xt)).item() * 100.0
